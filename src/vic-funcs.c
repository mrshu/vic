...
struct vic_func_t vic_all_funcs[VIC_ALL_FUNCS_COUNT] = {
    { "set", vic_set },
    { "get", vic_get }
};
struct vic_func_t *vic_funcs = vic_all_funcs + VIC_INTERN_FUNCS_COUNT;
...
int8_t vic_fn_intern_call(const char *raw_name)
{
    ...
    uint8_t i;
    for (i = 0; i < VIC_INTERN_FUNCS_COUNT; i++) {
        if (strcmp(name, vic_all_funcs[i].name) == 0) {
            /* found function to call */
            (*(vic_all_funcs[i].p_func))();
            return VIC_ERR_NO;
        }
    }
    return VIC_ERR_INVALID_NAME;
